...
export function AuthForm({ next }: { next: string }) {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="w-full max-w-sm space-y-4 rounded-[var(--radius)] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm sm:p-6"
    >
      ...
      <label className="block text-sm">
        البريد الإلكتروني
        <Input name="email" type="email" required dir="ltr" autoComplete="email" className="mt-1" />
      </label>
      <label className="block text-sm">
        كلمة المرور
        <Input name="password" type="password" required minLength={8} dir="ltr" className="mt-1" />
      </label>
