...
export interface HeroProps {
  ...
  /** عمل فني متكامل (خلفية + المنتج) يُرسم النص الحي فوقه ليبدو قطعة واحدة. */
 ...
 * variants: centered (نص في المنتصف) | split (لوحة إعلانية واحدة: عمل فني + نص حي فوقه) | banner (شريط مضغوط).
 */
export function Hero({ title, subtitle, cta, artwork, variant = "split" }: HeroProps) {
  const surface = "overflow-hidden rounded-card border border-[var(--card-border)] bg-[var(--card-bg)] shadow-card";
  const onArt = variant === "split" && Boolean(artwork);
  const text = (
    <div className={cn("flex flex-col gap-4 sm:gap-5", variant === "centered" && "items-center text-center", onArt && "gap-2.5 items-center text-center sm:items-start sm:text-start")}>
      ...
      {subtitle && (
        <p className={cn("max-w-lg leading-relaxed", onArt ? "text-brand-fg/80 text-xs sm:text-lg" : "text-ink-secondary", variant === "banner" && "text-sm", variant === "centered" && "text-base sm:text-lg", onArt && "line-clamp-3")}>
          {subtitle}
        </p>
      )}
      {cta && (
        <Link
          ...
          className={buttonClasses({
            ...
            className: cn("mt-1 w-fit gap-2", onArt && "min-h-10 bg-surface px-5 text-sm text-ink shadow-lg hover:bg-surface hover:brightness-95 sm:min-h-12 sm:px-7 sm:text-base"),
          })}
        >
          {cta.label}
