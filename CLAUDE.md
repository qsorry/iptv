...
- قاعدة عامة لكل الاختيارات: لا تستخدم `<select>` الأصلي ولا قوائم منسدلة. كل منتقي خيار/تصنيف/باقة/حالة
  (في المتجر ولوحة الإدارة والمعاينات الثابتة) يُعرض كورقة سفلية (Bottom Sheet) تنزلق من الأسفل عبر مكوّن
  `Select` في `src/components/ui/select.tsx` (يرسل القيمة عبر input مخفي فيعمل داخل نماذج الخادم).
  في معاينات HTML الثابتة (`scripts/ssouq-catalog/*build.py`) يُستخدم نفس النمط: زر `.picker` + `.sheet`.
