...
`process-events` يشغّل أيضاً (مرة كل 6 ساعات على الأكثر) إيقاف مزوّدي المحتوى الذين انتهت مستنداتهم المطلوبة،
فلا يحتاج مهمة Coolify مستقلة. `POST /api/internal/review-providers` باقٍ للتشغيل اليدوي بنفس الترويسة.
